# Szanse na odbudowę – Jezioro Skulska Wieś

Opisowa ocena szans na odbudowę poziomu wody w perspektywie **kolejnych 12 miesięcy**, na podstawie prognozy z symulacji wariantów pogodowych (opad i temperatura z percentyli historycznych) oraz szacowanych szans realizacji każdego wariantu (zgodność z ostatnimi 12 miesiącami pomiarowymi).

**Stan odniesienia:** koniec stycznia 2026 – poziom 85.758 m n.p.m.

**Porównanie prognoz** (najbardziej prawdopodobny scenariusz, średnia ważona):

| Model | Poziom koniec (m n.p.m.) | Zmiana (cm) |
|-------|-------------------------------------|-------------|
| Drenażowy (po zmianie reżimu) | 85.802 | +4.4 |
| Naturalny (sprzed drenażu) | 86.044 | +28.6 |

**Prognoza na koniec stycznia 2027** (model drenażowy): poziom **85.802 m n.p.m.**, czyli **wzrost o 4.4 cm** w stosunku do stanu odniesienia.

W zależności od warunków pogodowych w nadchodzącym roku prognozowana zmiana poziomu w 12 miesięcy (model drenażowy) mieści się w zakresie od **-35.8 cm** do **+43.1 cm**. Najwyższą szansę realizacji (16.1%) ma jeden z wariantów pogodowych; szczegóły wszystkich wariantów i szans – w raporcie [Prognoza 12 mies.](#docs-skulskawies-prognoza).

**Szanse na odbudowę w perspektywie roku (model drenażowy):** prognoza **modelu drenażowego** wskazuje na **stabilizację lub niewielki przyrost** poziomu w ciągu roku; szanse na odbudowę w tej perspektywie są umiarkowane.

**Szanse na odbudowę w perspektywie roku (model naturalny):** prognoza **modelu naturalnego** (sprzed drenażu) wskazuje na **możliwy przyrost** poziomu w ciągu roku.

Szacunki długoterminowej odbudowy (lata do zamknięcia luki po ewentualnym zaniku drenażu) – patrz [Zanik drenażu](#docs-skulskawies-zanik-drenazu).

*Wygenerowano: 2026-02-15 21:15*