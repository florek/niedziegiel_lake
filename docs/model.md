# Model prognozy zmiany poziomu jezior (Budzisławskie, Niedzięgiel, Powidzkie, Suszewskie, Wilczyńskie)
...
Obsługiwane jeziora: **Jezioro Budzisławskie** (`budzislawskie`), **Jezioro Niedzięgiel** (`niedziegiel`), **Jezioro Ostrowskie** (`ostrowskie`), **Jezioro Powidzkie** (`powidzkie`), **Jezioro Suszewskie** (`suszewskie`), **Jezioro Wilczyńskie** (`wilczynskie`). Dla każdego jeziora osobny plik danych i osobny wytrenowany model.

## Dane wejściowe

Pliki CSV w `data/`: `data/budzislawskie_data.csv`, `data/niedziegiel_data.csv`, `data/ostrowskie_data.csv`, `data/powidzkie_data.csv`, `data/suszewskie_data.csv`, `data/wilczynskie_data.csv`. Modele: `data/{id}_model.pkl`.
...
- **Podział:** temporalny, per jezioro. Niedzięgiel, Powidzkie: trening do 2013, test 2014–2023. Budzisławskie, Ostrowskie, Suszewskie, Wilczyńskie (oddziaływanie leja depresji): trening do 2003-02 (luty 2003), test od 2003-03 (`TRAIN_END_YEAR_BY_LAKE`, `TRAIN_END_MONTH_BY_LAKE`).
