...
def _project_recovery(
...
) -> tuple[list[str], list[float], list[float], list[str], list[float], list[float]]:
    ...
    dates_hist = [r["data"] for r in rows[-AVG_METEO_YEARS * 12 :]]
    model_hist = [float(r["wysokosc_model"]) for r in rows[-AVG_METEO_YEARS * 12 :]]
    actual_hist = [float(r["wysokosc_rzeczywista"]) for r in rows[-AVG_METEO_YEARS * 12 :]]
    dates_proj, model_proj, actual_proj = [], [], []
    while year < PROJECTION_END_YEAR:
        month = month + 1
        if month > 12:
            month, year = 1, year + 1
        if year >= PROJECTION_END_YEAR:
            break
        ...
        if year < RECOVERY_START_YEAR or (year == RECOVERY_START_YEAR and month < RECOVERY_START_MONTH):
            level_actual = level_actual_start + slope_actual_m_per_year * (t_proj - t_last)
        else:
            if not recovery_started:
                recovery_started = True
                gap_remaining = max(0.0, level_model - (level_actual_start + slope_actual_m_per_year * (t_proj - t_last)))
        ...
        if recovery_started and level_actual >= level_model - 1e-6:
            break
    return dates_hist, model_hist, actual_hist, dates_proj, model_proj, actual_proj
...
def build_report(results: list[dict], wznios_by_lake: dict[str, dict]) -> str:
    lines = [
        ...
        f"- **Projekcja odbudowy:** scenariusz modelowy przy **średnich warunkach z ostatnich {AVG_METEO_YEARS} lat (sezonowość)**. Do {RECOVERY_START_MONTH}/{RECOVERY_START_YEAR} poziom rzeczywisty – **trend z ostatnich 5 lat** rzeczywistych pomiarów; od {RECOVERY_START_MONTH}/{RECOVERY_START_YEAR} **wznios wód gruntowych** (cm/rok z CSV lub 5 cm/rok). Wykres do momentu zrównania z modelem (koniec odbudowy).",
        "",
        "---",
        "",
    ...
    for r in results:
        ...
        lines.append("")
        lines.append(f"![Odbudowa {r['lake_id']}](figures_odbudowa/odbudowa_{r['lake_id']}.png)")
        lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## Uwagi")
...
def _build_single_lake_report(r: dict, wznios_row: dict | None = None) -> str:
    ...
    lines = [
        ...
        f"- Projekcja: wznios wód gruntowych (cm/rok), start odbudowy od {RECOVERY_START_MONTH}/{RECOVERY_START_YEAR}, maks. do {PROJECTION_END_YEAR}.",
        "",
    ]
    if wznios_row:
    ...
    lines.extend([
        ...
        "",
        f"![Odbudowa](figures_odbudowa/odbudowa_{lid}.png)",
    ])
    return "\n".join(lines)
...
def main():
    ...
    for lake_id in lake.LAKES:
        ...
        dates_hist, model_hist, actual_hist, dates_proj, model_proj, actual_proj = _project_recovery(
            lake_id=lake_id,
            rows=rows,
            df=df,
            model=model,
    ...
    for r in results:
        ...
        wznios_row = wznios_by_lake.get(short)
        path = DOCS_DIR / f"{ZANIK_DRENAZU_PREFIX}_{r['lake_id']}.md"
